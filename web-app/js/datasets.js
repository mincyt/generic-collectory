...
function appendResource(value) {
    ...
    // row B
    $rowB.append('<span><strong class="resultsLabelFirst">Tipo de recurso: </strong>' + value.resourceType + '</span>');  // resource type
    $rowB.append('<span><strong class="resultsLabel">Licencia: </strong>' + (value.licenseType == null ? '' : value.licenseType) + '</span>'); // license type
    //$rowB.append('<span><strong class="resultsLabel">Versi&oacute;n de la licencia: </strong>' + (value.licenseVersion == null ? '' : value.licenseVersion) + '</span>'); // license version
    if (value.resourceType == 'records') {
        $rowB.append('<span class="viewRecords"><a title="' + jQuery.i18n.prop('datasets.js.appendresource03') + '" href="' + biocacheUrl + '/occurrences/search?q=data_resource_uid:' + value.uid + '">Ver registros</a></span>'); // records link
    }
    ...
    if (value.contentTypes != null) {
        $rowC.append('<span><strong class="resultsLabel">El contenido incluye:</strong></span>'); // label for content types
